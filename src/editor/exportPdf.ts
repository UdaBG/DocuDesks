 ...
 * content in every viewer, so whiteout/retype covers drawn into the content
 * stream can never hide them. Flatten them instead: draw each annotation's
 * normal appearance into the content stream (identical pixels), then drop the
 * annotation object — covers drawn afterwards land on top. Links stay live.
 */
function flattenAnnotations(page: PDFPage): void {
  ...
  for (let i = 0; i < annots.size(); i++) {
    ...
    try {
      ...
      const m = mArr ? [0, 1, 2, 3, 4, 5].map((j) => mArr.lookup(j, PDFNumber).asNumber()) : [1, 0, 0, 1, 0, 0]
